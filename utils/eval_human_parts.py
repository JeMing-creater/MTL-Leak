...
def eval_human_parts_predictions(database, save_dir, overfit=False,definition_db=None):
    ...
    if database == 'PASCALContext':
        ...
        if definition_db !=None:
            gt_set = ['train']
            tdb = PASCALContext(split=gt_set, do_edge=False, do_human_parts=True, do_semseg=False,
