...
import utils.jaccard as evaluation
    ...
    if type(loader) == type([]):
        ...
        for sample in loader:
            ...
            for j in range(0, sample['image'].shape[0]):
                if len(sample['sal'][j].unique()) == 1 and sample['sal'][j].unique() == 255:
                    continue
                filename = os.path.join(folder, sample["meta"]["image"][j] + '.png')
                try:
                    mask = np.array(Image.open(filename)).astype(np.float32) / 255.

                    # gt = cv2.resize(sample["sal"][j][0].cpu().data.numpy(),
                    #                 dsize=(sample['meta']['im_size'][1][j], sample['meta']['im_size'][0][j]),
                    #                 interpolation=0)
                    gt = np.array(Image.open(
                        './datasets/PASCAL_MT/sal_distill/{}.png'.format(sample['meta']['image'][j]))).astype(
                        np.float32) / 255.
                except:
                    continue
...
def eval_sal_predictions(database, save_dir, overfit=False, definition_db=None):
