...
def eval_semseg_predictions(database, save_dir, overfit=False, definition_db=None):
    ...
    if database == 'NYUD':
        ...
        has_bg = False
        gt_set = 'val'
        db = NYUD_MT(split=gt_set, do_semseg=True, overfit=overfit,download=False)
        if definition_db != None:
            gt_set = 'train'
            tdb = NYUD_MT(split=gt_set, do_semseg=True, overfit=overfit,download=False)
            db = db + tdb
        # Dataloaders
        ...
        db = PASCALContext(split=gt_set, do_edge=False, do_human_parts=False, do_semseg=True,
                           do_normals=False, overfit=overfit,download=False)
        if definition_db != None:
            gt_set = ['train']
            tdb = PASCALContext(split=gt_set, do_edge=False, do_human_parts=False, do_semseg=True,
                                do_normals=False, overfit=overfit,download=False)
            db = db + tdb
    else:
        raise NotImplementedError
